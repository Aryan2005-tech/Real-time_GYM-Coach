...
class VoicePipeline:
    ...
    def process_event(self, event, exercise, metrics):
        try:
            ...
            logger.error(f"Voice pipeline process_event failed: {e}")
            return None
        ...
        <script>
            (function() {{
                try {{
                    ...
                    // Stop any previously playing coach audio
                    if (pw._coachAudio) {{
                        pw._coachAudio.pause();
                        pw._coachAudio.currentTime = 0;
                    }}
                    // Create audio in parent window context so it survives iframe destruction
                    pw._coachAudio = new pw.Audio("data:audio/mp3;base64,{b64}");
                    pw._coachAudio.play().catch(function(e) {{
                        console.log('Coach audio autoplay blocked:', e);
                    }});
                }} catch(e) {{
                    console.log('Coach audio error:', e);
                }}
            }})();
        </script>
        """,
